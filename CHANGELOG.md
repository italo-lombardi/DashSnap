...
### Fixed
- Path traversal hardened: output file paths validated via `os.path.realpath` + `startswith(safe_root + os.sep)` — raises `RuntimeError` if path escapes the output directory.
